...
\hspace{1cm} \textit{Deadlock Prevention} adalah teknik yang digunakan untuk mencegah sistem memasuki keadaan buntu (\textit{deadlock}). Dengan berusaha memastikan setidaknya  satu dari keempat kondisi \textit{deadlock} tidak terpenuhi. 4 kondisi yang menyebabkan terjadinya \textit{deadlock}, yakni:
\begin{itemize}
...
\begin{itemize}
    \item Sebuah sistem operasi memiliki tiga jenis sumber daya: R1, R2, dan R3. Ada empat proses yang membutuhkan sumber daya tersebut, yaitu P1, P2, P3, dan P4. Setiap proses hanya dapat meminta satu sumber daya dalam satu waktu. Identifikasi jenis penyebab \textit{deadlock} apa dan jelaskan penyebab terjadinya \textit{deadlock} sertakan gambar sederhana bagaimana \textit{deadlock} terjadi!
    
    Jawaban: Kondisi ini termasuk kedalam  \textit{Hold-and-Wait}, karena siklus dari ketiga proses saling terjebak menuggu satu sama lain, sehingga tidak ada proses yang dapat melanjutkan eksekusinya. 
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{asset/gambar1.drawio.png} 
    \caption{Diagram sederhana dari proses \textit{Hold-and-Wait Conditions}.}
    \label{fig:contoh_gambar}
\end{figure}
